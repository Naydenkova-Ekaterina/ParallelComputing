...
void map_array1(double * array1, int size) {
  ...
  #pragma omp parallel
  for
  default (none) private(i) shared(array1, size) schedule(SCHEDULE_TYPE, CHUNCK_SIZE)
  for (i = 0; i < size; i++) {
...
void copy_array2(double * array2, double * array2_copy, int size) {
  ...
  #pragma omp parallel
  for
  default (none) private(i) shared(array2, array2_copy, size) schedule(SCHEDULE_TYPE, CHUNCK_SIZE)
  for (i = 0; i < size; i++) {
...
void merge(double * array1, double * array2, int size) {
  ...
  #pragma omp parallel
  for
  default (none) private(i) shared(array1, array2, size) schedule(SCHEDULE_TYPE, CHUNCK_SIZE)
  for (i = 0; i < size; i++) {
...
double reduce(double * array, int size) {
  ...
  #pragma omp parallel
  for
  default (none) private(i) shared(size, min, array) reduction(+: res) schedule(SCHEDULE_TYPE, CHUNCK_SIZE)
  for (i = 0; i < size; i++) {
